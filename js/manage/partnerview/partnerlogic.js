...
$('#partnerform').find('input, select, textarea').on('change keyup', function() {
    $('#invalid').hide();
});
...
function SetPartnerImage()
{
	if (confirm("Weet u zeker dat de afbeelding wilt wijzigen?"))
	{
		...
		$.ajax(
		{
	        ...
	        processData: false,
            success: function () {
                var successMessage = "De afbeelding is succesvol gewijzigd.";
...
function DeletePartner()
{
	if (confirm("Weet u zeker dat u deze partner wilt verwijderen?"))
	{
		...
		$.ajax(
		{
			...
			data: data,
            success: function () {
                var successMessage = "Partner succesvol verwijderd";
                localStorage.setItem("successMessage", successMessage);
